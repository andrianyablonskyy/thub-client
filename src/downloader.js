 ...
 * @description Downloads and verifies firmware/test artifacts (sha256) before a job runs
 *
...
async function downloadAndExtractTests(spec, jobDir, artifactoryCfg){
  ...
  await new Promise((resolve, reject) => {
    execFile('tar', ['-xzf', archive, '-C', testsDir], (err, stdout, stderr) => {
      if (err){
        return reject(new Error(`Extracting test package failed: ${stderr || err.message}`));
      }
      resolve();
    });
  });
